    ...
    public void create(Permisos permisos) throws PreexistingEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (RolHasPermisos rolHasPermisosListRolHasPermisos : permisos.getRolHasPermisosList()) {
                Permisos oldCodPermisoOfRolHasPermisosListRolHasPermisos = rolHasPermisosListRolHasPermisos.getCodPermiso();
                rolHasPermisosListRolHasPermisos.setCodPermiso(permisos);
                rolHasPermisosListRolHasPermisos = em.merge(rolHasPermisosListRolHasPermisos);
                if (oldCodPermisoOfRolHasPermisosListRolHasPermisos != null) {
                    oldCodPermisoOfRolHasPermisosListRolHasPermisos.getRolHasPermisosList().remove(rolHasPermisosListRolHasPermisos);
                    oldCodPermisoOfRolHasPermisosListRolHasPermisos = em.merge(oldCodPermisoOfRolHasPermisosListRolHasPermisos);
                }
            }
            utx.commit();
        } catch (Exception ex) {
        ...
        try {
            ...
            for (RolHasPermisos rolHasPermisosListOldRolHasPermisos : rolHasPermisosListOld) {
                if (!rolHasPermisosListNew.contains(rolHasPermisosListOldRolHasPermisos)) {
                    ...
                    illegalOrphanMessages.add("You must retain RolHasPermisos " + rolHasPermisosListOldRolHasPermisos + " since its codPermiso field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (RolHasPermisos rolHasPermisosListNewRolHasPermisos : rolHasPermisosListNew) {
                if (!rolHasPermisosListOld.contains(rolHasPermisosListNewRolHasPermisos)) {
                    Permisos oldCodPermisoOfRolHasPermisosListNewRolHasPermisos = rolHasPermisosListNewRolHasPermisos.getCodPermiso();
                    rolHasPermisosListNewRolHasPermisos.setCodPermiso(permisos);
                    rolHasPermisosListNewRolHasPermisos = em.merge(rolHasPermisosListNewRolHasPermisos);
                    if (oldCodPermisoOfRolHasPermisosListNewRolHasPermisos != null && !oldCodPermisoOfRolHasPermisosListNewRolHasPermisos.equals(permisos)) {
                        oldCodPermisoOfRolHasPermisosListNewRolHasPermisos.getRolHasPermisosList().remove(rolHasPermisosListNewRolHasPermisos);
                        oldCodPermisoOfRolHasPermisosListNewRolHasPermisos = em.merge(oldCodPermisoOfRolHasPermisosListNewRolHasPermisos);
                    }
                }
            }
            utx.commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (RolHasPermisos rolHasPermisosListOrphanCheckRolHasPermisos : rolHasPermisosListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Permisos (" + permisos + ") cannot be destroyed since the RolHasPermisos " + rolHasPermisosListOrphanCheckRolHasPermisos + " in its rolHasPermisosList field has a non-nullable codPermiso field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
