    ...
    public void create(Eps eps) throws PreexistingEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Paciente pacienteListPaciente : eps.getPacienteList()) {
                Eps oldIdEpsOfPacienteListPaciente = pacienteListPaciente.getIdEps();
                pacienteListPaciente.setIdEps(eps);
                pacienteListPaciente = em.merge(pacienteListPaciente);
                if (oldIdEpsOfPacienteListPaciente != null) {
                    oldIdEpsOfPacienteListPaciente.getPacienteList().remove(pacienteListPaciente);
                    oldIdEpsOfPacienteListPaciente = em.merge(oldIdEpsOfPacienteListPaciente);
                }
            }
            utx.commit();
        } catch (Exception ex) {
        ...
        try {
            ...
            for (Paciente pacienteListOldPaciente : pacienteListOld) {
                if (!pacienteListNew.contains(pacienteListOldPaciente)) {
                    ...
                    illegalOrphanMessages.add("You must retain Paciente " + pacienteListOldPaciente + " since its idEps field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Paciente pacienteListNewPaciente : pacienteListNew) {
                if (!pacienteListOld.contains(pacienteListNewPaciente)) {
                    Eps oldIdEpsOfPacienteListNewPaciente = pacienteListNewPaciente.getIdEps();
                    pacienteListNewPaciente.setIdEps(eps);
                    pacienteListNewPaciente = em.merge(pacienteListNewPaciente);
                    if (oldIdEpsOfPacienteListNewPaciente != null && !oldIdEpsOfPacienteListNewPaciente.equals(eps)) {
                        oldIdEpsOfPacienteListNewPaciente.getPacienteList().remove(pacienteListNewPaciente);
                        oldIdEpsOfPacienteListNewPaciente = em.merge(oldIdEpsOfPacienteListNewPaciente);
                    }
                }
            }
            utx.commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Paciente pacienteListOrphanCheckPaciente : pacienteListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Eps (" + eps + ") cannot be destroyed since the Paciente " + pacienteListOrphanCheckPaciente + " in its pacienteList field has a non-nullable idEps field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
