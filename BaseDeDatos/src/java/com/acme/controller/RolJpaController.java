    ...
    public void create(Rol rol) throws PreexistingEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (RolHasPermisos rolHasPermisosListRolHasPermisos : rol.getRolHasPermisosList()) {
                Rol oldRolcodrolOfRolHasPermisosListRolHasPermisos = rolHasPermisosListRolHasPermisos.getRolcodrol();
                rolHasPermisosListRolHasPermisos.setRolcodrol(rol);
                rolHasPermisosListRolHasPermisos = em.merge(rolHasPermisosListRolHasPermisos);
                if (oldRolcodrolOfRolHasPermisosListRolHasPermisos != null) {
                    oldRolcodrolOfRolHasPermisosListRolHasPermisos.getRolHasPermisosList().remove(rolHasPermisosListRolHasPermisos);
                    oldRolcodrolOfRolHasPermisosListRolHasPermisos = em.merge(oldRolcodrolOfRolHasPermisosListRolHasPermisos);
                }
            }
            utx.commit();
        } catch (Exception ex) {
        ...
        try {
            ...
            for (RolHasPermisos rolHasPermisosListOldRolHasPermisos : rolHasPermisosListOld) {
                if (!rolHasPermisosListNew.contains(rolHasPermisosListOldRolHasPermisos)) {
                    ...
                    illegalOrphanMessages.add("You must retain RolHasPermisos " + rolHasPermisosListOldRolHasPermisos + " since its rolcodrol field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (RolHasPermisos rolHasPermisosListNewRolHasPermisos : rolHasPermisosListNew) {
                if (!rolHasPermisosListOld.contains(rolHasPermisosListNewRolHasPermisos)) {
                    Rol oldRolcodrolOfRolHasPermisosListNewRolHasPermisos = rolHasPermisosListNewRolHasPermisos.getRolcodrol();
                    rolHasPermisosListNewRolHasPermisos.setRolcodrol(rol);
                    rolHasPermisosListNewRolHasPermisos = em.merge(rolHasPermisosListNewRolHasPermisos);
                    if (oldRolcodrolOfRolHasPermisosListNewRolHasPermisos != null && !oldRolcodrolOfRolHasPermisosListNewRolHasPermisos.equals(rol)) {
                        oldRolcodrolOfRolHasPermisosListNewRolHasPermisos.getRolHasPermisosList().remove(rolHasPermisosListNewRolHasPermisos);
                        oldRolcodrolOfRolHasPermisosListNewRolHasPermisos = em.merge(oldRolcodrolOfRolHasPermisosListNewRolHasPermisos);
                    }
                }
            }
            utx.commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (RolHasPermisos rolHasPermisosListOrphanCheckRolHasPermisos : rolHasPermisosListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Rol (" + rol + ") cannot be destroyed since the RolHasPermisos " + rolHasPermisosListOrphanCheckRolHasPermisos + " in its rolHasPermisosList field has a non-nullable rolcodrol field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
