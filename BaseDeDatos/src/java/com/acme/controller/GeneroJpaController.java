    ...
    public void create(Genero genero) throws PreexistingEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Paciente pacienteListPaciente : genero.getPacienteList()) {
                Genero oldIdGeneroOfPacienteListPaciente = pacienteListPaciente.getIdGenero();
                pacienteListPaciente.setIdGenero(genero);
                pacienteListPaciente = em.merge(pacienteListPaciente);
                if (oldIdGeneroOfPacienteListPaciente != null) {
                    oldIdGeneroOfPacienteListPaciente.getPacienteList().remove(pacienteListPaciente);
                    oldIdGeneroOfPacienteListPaciente = em.merge(oldIdGeneroOfPacienteListPaciente);
                }
            }
            utx.commit();
        } catch (Exception ex) {
        ...
        try {
            ...
            for (Paciente pacienteListOldPaciente : pacienteListOld) {
                if (!pacienteListNew.contains(pacienteListOldPaciente)) {
                    ...
                    illegalOrphanMessages.add("You must retain Paciente " + pacienteListOldPaciente + " since its idGenero field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Paciente pacienteListNewPaciente : pacienteListNew) {
                if (!pacienteListOld.contains(pacienteListNewPaciente)) {
                    Genero oldIdGeneroOfPacienteListNewPaciente = pacienteListNewPaciente.getIdGenero();
                    pacienteListNewPaciente.setIdGenero(genero);
                    pacienteListNewPaciente = em.merge(pacienteListNewPaciente);
                    if (oldIdGeneroOfPacienteListNewPaciente != null && !oldIdGeneroOfPacienteListNewPaciente.equals(genero)) {
                        oldIdGeneroOfPacienteListNewPaciente.getPacienteList().remove(pacienteListNewPaciente);
                        oldIdGeneroOfPacienteListNewPaciente = em.merge(oldIdGeneroOfPacienteListNewPaciente);
                    }
                }
            }
            utx.commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Paciente pacienteListOrphanCheckPaciente : pacienteListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Genero (" + genero + ") cannot be destroyed since the Paciente " + pacienteListOrphanCheckPaciente + " in its pacienteList field has a non-nullable idGenero field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
